...
def main(args=None):
    ...
    try:
        ...
        while rclpy.ok():
            ...
            if current_state == "END POSITION":
                if t - state_start_time == 0:
                    ...
                    try:
                        ...
                        # go back to starting posture
                        tsid_wrapper.setPostureRef(conf.q_home)
            ...
            # current_com = tsid_wrapper.comState().pos()
            ...
            if (current_state == "WALKING"):
                ...
                current_com = tsid_wrapper.comState().pos()
                desired_com = current_com.copy()
                desired_com[0] = desired_com[0] + 0.01
                desired_com[2] = current_com[2]

                tsid_wrapper.setComRefState(desired_com)
            # else:
            #     rf = tsid_wrapper.get_placement_RF().translation
            #     lf = tsid_wrapper.get_placement_LF().translation
            #     mid_feet = 0.5 * (rf + lf)
            #     desired_com = mid_feet.copy()
            #     desired_com[0] = desired_com[0] + 0.02

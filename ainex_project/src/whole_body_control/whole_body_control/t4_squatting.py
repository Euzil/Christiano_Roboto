...
class T4StandingRobotSimNode(Node):
    def __init__(self):
        ...
        self.lf_move_up = False
        self.shift_com = False
    ...
    def timer_callback(self):
        ...
        # check if it is time to shift COM
        if (not self.shift_com and self.t > 3):

            self.get_logger().info("shift com")

            # set state variable
            self.shift_com = True

            # get current COM position
            p_com_current = self.tsid_wrapper.comState().value()

            # get current right foot position
            rf_position = self.tsid_wrapper.get_placement_RF().translation

            # create new desired position for COM
            self.p_com_reference = np.zeros((3,))
            self.p_com_reference[:2] = rf_position[:2]
            self.p_com_reference[2] = p_com_current[2]

            # shift COM to right leg's x and y position while maintaining same hight
            self.tsid_wrapper.setComRefState(self.p_com_reference)

        # check if it is time to lift the left foot
        if (not self.lf_move_up and self.t > 6):

            self.get_logger().info("lift foot")

            # set state variable to true
            self.lf_move_up = True

            # remove left foot contact
            self.tsid_wrapper.remove_contact_LF()

            # get current left foots position
            lf_pose_current = self.tsid_wrapper.get_placement_LF()

            # define the desired left foot position such that it is 0.3 over the current position.
            lf_pose_current.translation[2] = lf_pose_current.translation[2] + 0.04

            # update reference pose
            self.tsid_wrapper.set_LF_pose_ref(lf_pose_current)

        if (self.t >= 10):

            # define amplitude and frequency of squatting
            amplitude = 0.007
            frequency = 0.5
            ...
            # calculate time since we startet doing squats
            time = self.t - 10 + conf.dt

            # create sine wave over time along the z axis and calculate its derivatives
            z_pos = amplitude * np.sin(omega * time)
            z_vel = omega * amplitude * np.cos(omega * time)
            z_acc = - omega**2 * amplitude * np.sin(omega * time)

            # self.p_com_reference holds zero point for sine wave
            p_com_desired = self.p_com_reference.copy()
            p_com_desired[2] = p_com_desired[2] + z_pos

            # create velocity vector
            v_com_desired = np.zeros((3,))
            v_com_desired[2] = z_vel

            # create acceleration vector
            a_com_desired = np.zeros((3,))
            a_com_desired[2] = z_acc

            # set new COM reference
            self.tsid_wrapper.setComRefState(p_com_desired, v_com_desired, a_com_desired)

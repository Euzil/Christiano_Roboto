...
class Talos:
    ...
    def __init__(self, simulator):
        ...
        self.robot = Robot(
            simulator=simulator,
            filename=conf.urdf,
            model=self.stack.model,  # Use TSIDWrapper's model
            basePosition=np.array([0, 0, 1.1]),  # Adjust initial height as needed
            q=conf.q_home,  # Use configured initial pose
            useFixedBase=False  # Talos is floating base
        )
    ...
    def publish(self):        
        ...
        # Get joint names - use Robot class method
        joint_msg.name = self.robot.actuatedJointNames()
        
        # Get joint state - get from Robot class
        ...
        # Publish feet wrenches - use TSIDWrapper to get contact forces
    ...
    def _update_zmp_estimate(self):
        ...
        """
        # Use TSIDWrapper to get contact forces
        contact_forces = self.stack.getContactForces()
        left_ft = contact_forces.get("left_foot", np.zeros(6))
        right_ft = contact_forces.get("right_foot", np.zeros(6))
    ...
    def _update_dcm_estimate(self):
        ...
        """
        # Use TSIDWrapper to get center of mass state
